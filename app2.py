...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://adminfurgones101:furgonescontrasenia101@furgones.ctgc2u6yy0tr.us-east-1.rds.amazonaws.com/furgofinder'
...
@app.route('/conductores/<int:id>', methods=['DELETE'])
...
@app.route('/conductores/<int:id>', methods=['PUT'])
...
@app.route('/conductores/<int:id>', methods=['GET'])
...
def login():
    ...
    # Busca al usuario en las tablas de Apoderado y Conductor
    apoderado = Apoderado.query.filter_by(correo=correo).first()
    conductor = Conductor.query.filter_by(correo=correo).first()

    user = apoderado or conductor  # Si existe en alguna de las dos tablas
    ...
    # Verifica que el rol esté correctamente configurado
...
#     app.run(debug=True)
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5000))  # Usa el puerto que asigna Heroku
